...
print("")
print("*** achievementfortoday_00.py started at", datetime.now())
print("")
...
###########################################################################
try:
    sqlfile = open('achievementfortoday_load_05.ddl', 'r')
    try:
...
###########################################################################
try:
    with open('achievementfortoday.json', 'r') as f:

        try:
            cur.copy_from(f, 'achievementfortoday_load_10.sql')
        except psycopg2.Error as e:
            e.pgcode
            print(e.pgerror)
...
###########################################################################
try:
    sqlfile = open('achievementfortoday_insert_15.sql', 'r')
...
###########################################################################
try:
    sqlfile = open('achievementfortoday_count_20.sql', 'r')
    cur.execute(sqlfile.read())
    rows = cur.fetchall()
    conn.commit()
except:
...
print("")
print("*** achievementfortoday_00.py finished at", datetime.now())
print("")
quit()
