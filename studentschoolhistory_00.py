...
print("")
print("*** studentschoolhistory_00.py started at", datetime.now())
print("")
...
###########################################################################
try:
    sqlfile = open('studentschoolhistory_load_05.ddl', 'r')
    try:
...
###########################################################################
try:
    os.system('psql -f studentschoolhistory_load_10.sql -d schools -U postgres')
...
###########################################################################
try:
    sqlfile = open('studentschoolhistory_insert_15.sql', 'r')
    try:
        cur.execute(sqlfile.read())
        conn.commit()
    except psycopg2.Error as e:
...
###########################################################################
try:
    sqlfile = open('studentschoolhistory_count_20.sql', 'r')
    cur.execute(sqlfile.read())
    rows = cur.fetchall()
    conn.commit()
except:
...
print("")
print("*** studentschoolhistory_00.py finished at", datetime.now())
print("")
quit()
