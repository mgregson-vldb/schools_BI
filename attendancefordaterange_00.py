...
print("")
print("*** attendancefordaterange_00.py started at", datetime.now())
print("")
...
###########################################################################
try:
    sqlfile = open('attendancefordaterange_load_05.ddl', 'r')
    try:
...
###########################################################################
try:
    with open('attendancefordaterange.json', 'r') as f:

        try:
            cur.copy_from(f, 'attendancefordaterange_load_10.sql')
        except psycopg2.Error as e:
            e.pgcode
            print(e.pgerror)
...
###########################################################################
try:
    sqlfile = open('attendancefordaterange_insert_15.sql', 'r')
...
###########################################################################
try:
    sqlfile = open('attendancefordaterange_count_20.sql', 'r')
    cur.execute(sqlfile.read())
    rows = cur.fetchall()
    conn.commit()
except:
...
print("")
print("*** attendancefordaterange_00.py finished at", datetime.now())
print("")
quit()
