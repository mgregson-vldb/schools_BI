...
print("")
print("*** timetablemode_00.py started at", datetime.now())
print("")
...
###########################################################################
try:
    sqlfile = open('timetablemode_load_05.ddl', 'r')
    try:
...
###########################################################################
try:
    with open('timetablemode.json', 'r') as f:

        try:
            cur.copy_from(f, 'timetablemode_load')
        except psycopg2.Error as e:
            e.pgcode
            print(e.pgerror)
...
###########################################################################
try:
    sqlfile = open('timetablemode_insert_15.sql', 'r')
...
###########################################################################
try:
    sqlfile = open('timetablemode_count_20.sql', 'r')
    cur.execute(sqlfile.read())
    rows = cur.fetchall()
    conn.commit()
except:
...
print("")
print("*** timetablemode_00.py finished at", datetime.now())
print("")
quit()
