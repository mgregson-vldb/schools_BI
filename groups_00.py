...
print("")
print("*** groups_00.py started at", datetime.now())
print("")
...
###########################################################################
try:
    sqlfile = open('groups_load_05.ddl', 'r')
    try:
...
###########################################################################
try:
    os.system('psql -f groups_10.sql -d schools -U postgres')
...
###########################################################################
try:
    sqlfile = open('groups_insert_15.sql', 'r')
    try:
        cur.execute(sqlfile.read())
        conn.commit()
    except psycopg2.Error as e:
...
###########################################################################
try:
    sqlfile = open('groups_count_20.sql', 'r')
    cur.execute(sqlfile.read())
    rows = cur.fetchall()
    conn.commit()
except:
...
print("")
print("*** groups_00.py finished at", datetime.now())
print("")
quit()
