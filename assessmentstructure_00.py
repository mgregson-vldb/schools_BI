...
print("")
print("*** assessmentstructure_00.py started at", datetime.now())
print("")
...
###########################################################################
try:
    sqlfile = open('assessmentstructure_load_05.ddl', 'r')
    try:
...
try:
    os.system('psql -f assessmentstructure_load_10.sql -d schools -U postgres')
...
###########################################################################
try:
    sqlfile = open('assessmentstructure_insert_15.sql', 'r')
...
###########################################################################
try:
    sqlfile = open('assessmentstructure_count_20.sql', 'r')
    cur.execute(sqlfile.read())
    rows = cur.fetchall()
    conn.commit()
except:
...
print("")
print("*** assessmentstructure_00.py finished at", datetime.now())
print("")
quit()
